        ...
        public Square[,] GenerateEmptyBoard(int rows, int cols)
        {
            ...
            for (int row = 0; row < rows; row++)
            {
                for (int col = 0; col < cols; col++)
                {
                    ...
                    if ((row + col) % Constants.BoardEl == 0)
                    {
                        square = new Square(row, col, "Yellow", new MathTask(row, col, null, "problem"));
                    }
                    else if ((row + col) % Constants.BoardEl == 1)
                    {
                        square = new Square(row, col, "Orange", new MathTask(row, col, null, "problem"));
                    }
                    else if ((row + col) % Constants.BoardEl == 2)
                    {
                        square = new Square(row, col, "Red", new MathTask(row, col, null, "problem"));
                    }
